	...
	ImU32 left_colour, ImU32 right_colour)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y), window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f) + ImVec2(0, 2*label_size.y));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x : 0.0f, 0.0f) + ImVec2(0, label_size.y / 2.0f));
    ...
    RenderTextClipped(frame_bb.Min - ImVec2((frame.Max.x - frame.Min.x), 0), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
